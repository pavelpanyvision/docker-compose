...
create_ssl() {
	...
	openssl req -new -x509 -subj "/CN=anyvisionCA" -extensions v3_ca -days 3650 -key $CA_CERTS_DIR/anyvisionCA.key.pem -sha256 -out $CA_CERTS_DIR/anyvisionCA.pem -config /etc/ssl/openssl.cnf -passin pass:$password
	...
	openssl x509 -req -extensions v3_req -days 3650 -sha256 -passin pass:$password -in $CA_CERTS_DIR/apigateway.anyvision.local.csr -CA $CA_CERTS_DIR/anyvisionCA.pem -CAkey $CA_CERTS_DIR/anyvisionCA.key.pem -CAcreateserial -out $CA_CERTS_DIR/apigateway.anyvision.local.crt -extfile /etc/ssl/openssl.cnf
