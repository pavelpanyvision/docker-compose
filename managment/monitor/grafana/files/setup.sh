...
PREFERENCE_PATH=${PREFERENCE_PATH:-/etc/grafana/files/preferences.json}
...
grafana_api() {
  ...
  cmd="curl -L -s -H \"Accept: application/json\" -H \"Content-Type: application/json\" -X ${verb} -k ${GRAFANA_URL}${url}"
...
install_dashboards() {
  ...
  do
    if [[ -f "${dashboard}" ]]; then
      ...
      dashboard_name=$(echo ${dashboard} | grep -o '[^/]*$')
      echo "Dashboard name: ${dashboard_name}"
      echo "{\"dashboard\": `cat $dashboard`}" > "/tmp/${dashboard_name}.wrapped"

      if grafana_api POST /api/dashboards/db "" "/tmp/${dashboard_name}.wrapped"; then
        echo "installed ok"
      else
        echo "install failed"
      fi

      #rm "/tmp/${dashboard_name}.wrapped"
    fi
  done
}
...
set_preferences(){
    ...
    if grafana_api PUT /api/org/preferences "" "${PREFERENCE_PATH}"; then
        echo "installed ok"
    else
        echo "install failed"
    fi
