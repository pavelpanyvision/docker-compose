...
SELECT track_id,source_id,creation_datetime,detection_type,features FROM detections_in_memory
WHERE creation_date >= (SELECT NVL((SELECT ADDDATE(MIN(creation_date), INTERVAL 1 DAY) FROM detections_in_memory),DATE(ADDDATE(NOW(),INTERVAL 1 DAY))))
UNION ALL
SELECT track_id,source_id,creation_datetime,detection_type,features FROM detections
WHERE creation_date < (SELECT NVL((SELECT ADDDATE(MIN(creation_date), INTERVAL 1 DAY) FROM detections_in_memory),DATE(ADDDATE(NOW(),INTERVAL 1 DAY))));
