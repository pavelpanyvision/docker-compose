...
def main():
    ...
    yml_file = os.path.join(os.getcwd(), 'docker-compose-gpu.yml')
    ...
    if mode == 'multicast':
        add_entries(st, 'extra_hosts', 'edge.tls.ai:{}'.format(edge_ip), ['api', 'backend', 'collate'])
        ...
        remove_entries(st, 'extra_hosts', 'edge.tls.ai:{}'.format(edge_ip), ['backend', 'collate'])
